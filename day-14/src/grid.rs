...
impl FallingGrid {
    ...
    fn are_under_free(&self, p: &Point) -> (bool, bool, bool) {
        ...
        let p_diag_left = Point { x: p_x_left, y: p.y + 1 };
        let p_directly_under = Point { x: p.x, y: p.y + 1 };
        let p_diag_right = Point { x: p.x + 1, y: p.y + 1 };
        (
            // diagonal left
    ...
    fn is_point_free(&self, p: &Point) -> bool {
        !self.filled.contains(p)
    }

    fn is_point_occupied(&self, p: &Point) -> bool {
        self.filled.contains(p)
    }
}
...
mod falling_grid_test {
    ...
    fn it_can_check_are_under_free() {
        ...
        assert_eq!(fg.are_under_free(&Point { x: 3, y: 0 }), (false, true, true));
        assert_eq!(fg.are_under_free(&Point { x: 2, y: 0 }), (false, false, true));
        assert_eq!(fg.are_under_free(&Point { x: 1, y: 0 }), (false, false, false));
    }
}
